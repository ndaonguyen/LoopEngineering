...
Vite + React 19 single-page app for the LoopEngineering service.

## Develop

```bash
# 1. Run the API (separate terminal) — serves https://localhost:7443
dotnet run --project ..      # from source/LoopEngineering.Api
...
On `dotnet publish` of `LoopEngineering.Api`, this build runs automatically and its
output is copied into `wwwroot` (see `LoopEngineering.Api.csproj`).
...
The loopengineering pages use `fetch` directly so this step is optional.
